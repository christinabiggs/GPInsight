...
input_data_dir <- here::here("data/input")
...
total_dataset <- read.csv(paste0(input_data_dir,"/GPDataCOMPLETE_2024.csv"))
...
bnf_freq_dist_plot <- ggplot(data = bnf_freq_dist, aes(x = bnf_proxy_code, y = freq)) +
  geom_point() +
...
ggsave(bnf_freq_dist_plot, 
       filename = "data/output/bnf_freq_dist.png",
       device = "png",
       height = 4, width = 6, units = "in")

plot(bnf_freq_dist_plot)

```

```{r plot-zoom}

bnf_freq_dist_plot_zoom <- ggplot(data = bnf_freq_dist, aes(x = bnf_proxy_code, y = freq)) +
  geom_point() +
...
ggsave(bnf_freq_dist_plot_zoom, 
       filename = "data/output/bnf_freq_dist_zoom.png",
       device = "png",
       height = 4, width = 6, units = "in")

plot(bnf_freq_dist_plot_zoom)
...
df_summary <- total_dataset %>%
  ...
  dplyr::filter(total_prescriptions_2024 > 5000)
...
raw_data_plot <- ggplot(data = df_summary, aes(x = total_prescriptions_2024, y = unique_bnf_2024, ymin=0)) +
  geom_point(colour="blue") +
  scale_fill_manual(values="blue") +
  scale_x_continuous(labels = label_comma()) +
  labs(title = "Prescription range versus rate - by surgery",
       y = "BNF Medication Range",
       x = "Average Yearly Prescription Rate in 2024") +
  theme_minimal()

plot(raw_data_plot)

ggsave(raw_data_plot, 
       filename = "data/output/raw_data_plot.png",
       device = "png",
       height = 4, width = 6, units = "in")
...
nls_log_fullrange <- nls(unique_bnf_2024 ~ a - b* exp(c * total_prescriptions_2024),
               data = df_for_fit_full_range,
               start = list(a = starting_val, 
                            b = starting_val, c=-0.00012))

df_summary$fullrange_fit <- predict(nls_log_fullrange, df_summary$total_prescriptions_2024)

new_x_fullrange <- data.frame(total_prescriptions_2024 = seq(min(df_summary$total_prescriptions_2024),
                                            2*max(df_summary$total_prescriptions_2024), 
                                            length.out = 373))
...
df_for_fit_full_range$log_fit_line <- predict(nls_log_fullrange, newdata = new_x_fullrange$total_prescriptions_2024)

asymptote_fullrange <- as.integer(max(new_x_fullrange$predicted_y))

refl_exp_fit_plot <- ggplot() +
  geom_point(data = df_summary, aes(x = total_prescriptions_2024, y = unique_bnf_2024, ymin=0,
                                    xmax=70000)) + 
  geom_ribbon(data = new_x_fullrange, aes(x = total_prescriptions_2024, ymin = lwr, ymax = upr),
              fill = "blue", alpha = 0.2) +
  geom_point(colour="blue") +
  scale_fill_manual(values="blue") +
  scale_x_continuous(labels = label_comma()) +
  geom_line(data = df_summary, aes(x = total_prescriptions_2024, y = fullrange_fit), colour = "red", alpha = 1) +
  labs(title = "Prescription data fitted with reflected exponential",
       y = "Unique BNF Codes",
       x = "Total Prescriptions in 2024") +
  theme_minimal()

plot(refl_exp_fit_plot)

ggsave(refl_exp_fit_plot, 
       filename = "data/output/refl_exp_fit_plot.png",
       device = "png",
       height = 4, width = 6, units = "in")
...
df_seq1 <- data.frame(value = seq(from=0, to= 18000, by= 2000))
df_seq2 <- data.frame(value = seq(from=20000, to= 200000, by= 20000))
df_seq3 <- data.frame(value = seq(from=200000, to= 800000, by= 200000))
...
n_rows <- i # This is the number of prescriptions chosen each year
n_cols <- 100 # This repeats the selection to get the mean and sd of distinct_counts.
min_val <- 1
max_val <- max(df_summary$unique_bnf_2024) # Subset of total from the fitted asymptote from reflected exponential
...
random_model_plot <- ggplot(data=df_summary,aes(x=total_prescriptions_2024,y=unique_bnf_2024,ymin=0)) +
  geom_point(colour="blue") +
  scale_fill_manual(values="blue") +
  scale_x_continuous(labels = label_comma()) +
  ...
  labs(title = "Simulated prescriptions - uniform random model",
       x = "Prescriptions issued per year",
       y = "Unique BNF Codes over one year") +
  theme_minimal()

ggsave(random_model_plot, 
       filename = "data/output/random_model_plot.png",
       device = "png",
       height = 4, width = 6, units = "in")


plot(random_model_plot)
...
fn_emulate_gp_yearly <- function(df_summary,bnf_freq_dist){
...
df_summary <- df_summary %>%
     dplyr::mutate(emulate_range_yearly = 0)

df_summary <- fn_emulate_gp_yearly(df_summary,bnf_freq_dist)

```
```{r plot-emulation}

emulate_gplot <- ggplot(data = df_summary, aes(x = total_prescriptions_2024, y = unique_bnf_2024)) +
    geom_point(colour="blue") +
    scale_fill_manual(values="blue") +
    scale_x_continuous(labels = label_comma()) +
    geom_line(data = df_summary, aes(x = total_prescriptions_2024, 
...
ggsave(emulate_gplot, 
       filename = "data/output/gp_emulation.png",
       device = "png",
       height = 4, width = 6, units = "in")
